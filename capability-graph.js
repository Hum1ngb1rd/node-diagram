...
class ExpandableNode extends Rect {
    ...
    // return {
    //     backgroundFill: '#fff',
    //     backgroundHeight: height/3,
    //     backgroundLineWidth: 1,
    //     backgroundRadius: 0,
    //     backgroundStroke: '#023047',
    //     backgroundWidth: height/3,
    //     cursor: 'pointer',
    //     fill: '#023047',
    //     fontSize: 54,
    //     text: collapsed ? '-' : '+',
    ...
    // };
    // }
    // drawButton(attributes, container) {
    //     const collapseStyle = this.getCollapseStyle(attributes);
    //     const expandBtn = this.upsert('collapse', Badge, collapseStyle, container);

    //     if (expandBtn && !Reflect.has(expandBtn, '__bind__')) {
    //         Reflect.set(expandBtn, '__bind__', true);
    //         expandBtn.addEventListener(CommonEvent.CLICK, () => {
    ...
    //             // const existingNode = graph.getNodeData(newNodeId);
    //             // console.log(existingNode);
    //             // if (existingNode) {
    //             //     // Toggle visibility instead of recreating
    //             //     const isVisible = graph.getItemState(existingNode, 'visible');
    //             //     graph.setItemState(existingNode, 'visible', !isVisible);
    //             //     graph.setItemState(`${nodeId}-edge`, 'visible', !isVisible);
    //             //     console.log("node detected");
    //             //     return;
    //             // }

    //             // // Get badge position relative to parent node
    //             // var [posX, posY] = graph.getElementPosition(nodeId);
    //             // console.log(posX + ' ' + posY);
    //             // var badgeX = posX + width / 2 + 250; // Position right of badge
    //             // var badgeY = posY;
    //             // console.log("test-0");
    //             // // Create new node
    //             // const newNode = {
    ...
    //             //     size: [500, 250],
    //             //     label: 'New Node',
    //             //     style: { fill: '#FFAA00', stroke: '#CC7700' },
    //             // };

    //             // // Create edge from badge to new node
    //             // const newEdge = {
    //             //     id: `${nodeId}-edge`,
    //             //     source: nodeId,
    //             //     target: newNodeId,
    ...
    getLinkTextStyle(attributes){
        ...
        return {
            ...
            cursor: 'pointer', // Makes text look clickable
            // textDecoration: 'underline', // Optional: Underline to indicate a link
        };        
    }
...
const rectStyle = {
    ...
    // labelPlacement: (d) => d.combo === 'stages' ? 'top' : 'bottom',
    // labelPlacement: (d) => d.combo === 'stages' ? 'left' : 'right',
    // labelOffsetX : (d) => d.combo === 'stages' ? -12 : 12,
    labelOffsetY: rectHeight/6,
    // labelOffsetY : (d) => d.combo === 'stages' ? -12 : 12,
    labelPlacement: 'center',
    labelFill: '#fff',
    labelFontWeight: 'bold',
...
const comboStyle = {
    ...
    //  fillOpacity: (d) => d.id === 'stages' ? 0 : 0.1,
    // strokeOpacity: (d) => d.id === 'stages' ? 0 : 1,
    lineDash: 40,
    lineWidth: 10,
    padding: 24,
    ...
    plugins: [
        {
          ...
          getContent: (e, items) => {
            ...
            // tooltipDiv.style.background = '#fff4d1';
            // tooltipDiv.style.background = 'rgb(206, 252, 23)';
            tooltipDiv.style.background = nodeColour;
            tooltipDiv.style.color = '#fff';
            tooltipDiv.style.borderRadius = '5px';
            // tooltipDiv.style.border = '10px solid #fc982d';
            ...
            details.forEach(({ title, value }) => {
                ...
                titleElement.style.color = '#FFDD44';  // Yellow title
                titleElement.style.fontSize = '14px';
                titleElement.style.fontWeight = 'bold';
                titleElement.style.textDecoration = 'underline';
                titleElement.style.marginRight = '5px';  // Space between title and value
                // titleElement.style.lineHeight = '1em';

                // Create value element
                const valueElement = document.createElement('span'); // Use <span> instead of <p>
                valueElement.innerText = value;
                valueElement.style.fontSize = '14px';
                valueElement.style.color = 'white';

                // Wrap both elements in a single line
                const lineContainer = document.createElement('p'); // Keep them inside a <p>
            ...
            linkElement.innerText = 'Click to view more...'; // Display link text
            ...
            return tooltipDiv;
            // return `<h2>Tooltip</h2><h2>${item.data.title}</h2><p>Lead Time: ${item.data.lead}</p><p>Requirements: ${item.data.requirements}</p><p>Description: ${item.data.description}</p>`;
            // let result = `<h2>Tooltip</h2>`;
            // items.forEach((item) => {
            //     result += `<h2>${item.data.title}</h2>`;
            //     result += `<p>Lead Time: ${item.data.lead}</p>`;
            //     result += `<p>Requirements: ${item.data.requirements}</p>`;
            //     result += `<p>Description: ${item.data.description}</p>`;
            // });
            // return result;
          },
        },
